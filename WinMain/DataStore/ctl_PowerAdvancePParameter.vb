...
'*************************************************************************************************************
' ctl_PowerAdvancePParameter - DataStore enabled UserControl
'
' Combines a TextBox & a Label in a UserControl to provide a Power Advance p value with units.
'
' ctl_PowerAdvancePParameter is subclassed from the Windows.Forms.UserControl to add interaction with
'       a PowerAdvanceP DataStore Property.
'*************************************************************************************************************
Imports DataStore

Public Class ctl_PowerAdvancePParameter
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        '
        'ctl_PowerAdvancePParameter
        '
        Me.Controls.Add(Me.TextBox)
        Me.Controls.Add(Me.UnitsLabel)
        Me.Name = "ctl_PowerAdvancePParameter"
...
#Region " Properties "
    '
    ' IsCalculated = True displays the value as a 'Constant'
    '
    Protected mIsCalculated As Boolean = False
...
#Region " Methods "
    '
    ' Get a copy of the Power Advance p parameter from DataStore
    '
    Private Function DataParameter() As PowerAdvancePParameter
        Dim _parameter As Parameter = mProperty.GetParameter

        If (_parameter.GetType Is GetType(PowerAdvancePParameter)) Then
            Dim _k As PowerAdvancePParameter = DirectCast(_parameter, PowerAdvancePParameter)
            Dim _copy As PowerAdvancePParameter = New PowerAdvancePParameter(_k)
            Return _copy
        Else
    ...
    ' Units
    '
    Public ReadOnly Property DisplayUnits() As PowerAdvancePParameter.P_Units
        Get
            Dim _powerAdvanceP As PowerAdvancePParameter = Me.DataParameter
            If (_powerAdvanceP.AltUnits = PowerAdvancePParameter.P_Units.NoUnits) Then
                Return PowerAdvancePParameter.DisplayUnits
            Else
                Return _powerAdvanceP.AltUnits
            End If
        End Get
    End Property
    '
...
#Region " UI Update Methods "
    ...
    Private Sub SaveNewValue()
        If (mProperty IsNot Nothing) Then
            ...
            If Not (mSavingNewValue) Then
                ...
                Dim _powerAdvanceP As PowerAdvancePParameter = Me.DataParameter
                Dim _value As Double = _powerAdvanceP.Value

                ' Get current display units
                Dim _units As PowerAdvancePParameter.P_Units = Me.DisplayUnits

                ' Don't process UI if parameter is a constant
                If Not (_powerAdvanceP.Source = ValueSources.Constant) Then
                    ' Process text input
                    If (TextBox.Text.Length = 0) Then
                        ' Text Box is empty; user set value to undefined
                        _powerAdvanceP.SetValue(0.0, _units, _powerAdvanceP.PowerAdvanceR)
                        _powerAdvanceP.Source = ValueSources.Undefined

                        mMyStore.MarkForUndo(mTranslator.tClearValue.Translated)
                        mProperty.SetParameter(_powerAdvanceP)
                        SetError("")
                    ...
                    Else
                        ...
                        If Not (TextBox.Text = mTextBoxText) Then
                            ...
                            If (Parsable()) Then
                                _value = ParsedValue()
                                _powerAdvanceP.SetValue(_value, _units, _powerAdvanceP.PowerAdvanceR)
                                _powerAdvanceP.Source = ValueSources.UserEntered

                                mMyStore.MarkForUndo(mTranslator.tSetValue.Translated)
                                mProperty.SetParameter(_powerAdvanceP)

                                ' Record change as Script command
                                DataStore.DataStore.RecordScript(_powerAdvanceP)
    ...
    Public Sub UpdateUI()
        If (mProperty IsNot Nothing) Then
            ...
            Dim _powerAdvanceP As PowerAdvancePParameter = Me.DataParameter
            Dim _value As String = _powerAdvanceP.Text(Me.DisplayUnits)
            Dim _units As String = PowerAdvancePParameter.P_UnitsText(Me.DisplayUnits)
            ...
            If ((_powerAdvanceP.Source = ValueSources.Constant) Or Me.IsCalculated) Then
            ...
            Else
                ...
                Select Case _powerAdvanceP.Source
    ...
    Public Sub CheckError()
        ...
        Dim _powerAdvanceP As PowerAdvancePParameter = Me.DataParameter   ' Get value from model object

        If (_powerAdvanceP IsNot Nothing) Then
            If Not (_powerAdvanceP.ValidateValue) Then
                Dim _value As Double = _powerAdvanceP.Value
                If (_value < _powerAdvanceP.MinValue) Then
                    _errMsg = String.Format("Value < {0}", _powerAdvanceP.MinValue)
                ElseIf (_powerAdvanceP.MaxValue < _value) Then
                    _errMsg = String.Format("{0} < Value", _powerAdvanceP.MaxValue)
...
#Region " UI Event Handlers "
    ...
    Protected Overridable Sub Units_MouseUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles UnitsLabel.MouseUp
        ...
        Select Case (e.Button)
            Case MouseButtons.Right
                ...
                Dim _numUnits As Integer = PowerAdvancePParameter.P_UnitsText.Length
                ...
                If (1 < _numUnits) Then
                    ...
                    For _idx As Integer = 0 To _numUnits - 1
                        UnitsMenu.MenuItems.Add(PowerAdvancePParameter.P_UnitsText(_idx), AddressOf UnitsMenu_Click)

                        If (PowerAdvancePParameter.P_UnitsText(_units) = PowerAdvancePParameter.P_UnitsText(_idx)) Then
    ...
    Private Sub UnitsMenu_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If (sender.GetType Is GetType(MenuItem)) Then
            ...
            ' Save the Alternate Display Units associated with this Power Advance p Parameter
            Dim pUnits As PowerAdvancePParameter.P_Units = CType(_powerAdvanceP.PUnits + _idx, PowerAdvancePParameter.P_Units)

            ' If User set units back to original, set Alt Units to None.
            '   This will enable units to follow UnitsSystem changes
            If (pUnits = PowerAdvancePParameter.DisplayUnits) Then
                pUnits = PowerAdvancePParameter.P_Units.NoUnits
            End If

            _powerAdvanceP.AltUnits = pUnits
